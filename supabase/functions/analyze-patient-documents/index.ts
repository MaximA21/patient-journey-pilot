...
serve(async (req) => {
  ...
  try {
    ...
    if (!documents || documents.length === 0) {
      return new Response(
        JSON.stringify({ success: false, message: 'No processed documents found for this patient' }),
    ...
    if (!medicalHistoryForm || !medicalHistoryForm.questions) {
      return new Response(
        JSON.stringify({ success: false, message: 'Medical history form not found or has no questions' }),
        { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
...
- When providing multiple possible answers, separate them with commas.`;
    ...
    // 7. Extract the JSON response and validate it
    let answersJson;
    try {
